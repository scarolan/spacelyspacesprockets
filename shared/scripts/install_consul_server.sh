...
CONSUL_FLAGS="-server -node=${NODENAME} -bootstrap-expect=${SERVER_COUNT} -join=${CONSUL_JOIN} -client=${IPADDR} -data-dir=/opt/consul/data -ui"
EOF
...
then
  ...
  # This next line appears to fail on CentOS7. 
